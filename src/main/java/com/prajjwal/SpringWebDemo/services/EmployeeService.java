...
//    }
    ...
    public EmployeeDTO getEmployeeById(Long id) {
        ifExistsByEmployeeId(id);
        Optional<EmployeeEntity> employeeEntity = employeeRepository.findById(id);
        return mapper.map(employeeEntity.get(), EmployeeDTO.class);
    }
    ...
    public EmployeeDTO updateEmployeeById(Long employeeId, EmployeeDTO employeeDTO) {
        ifExistsByEmployeeId(employeeId);
    ...
    5. And now, we will now use our input Map update to update the particular values that are sent by
       user instead of whole object.
    ...
    9. Our fields in Entity class is private, so we need to change accessibility
        to true, so that we can directly update the value.
    ...
    11. And we are saving the Update Object to our DB using Repository, and converting the
        employeeEntity to Employee DTO and returning it
    */
    ...
    public void ifExistsByEmployeeId(Long employeeId) {
